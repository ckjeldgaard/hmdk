...
<?php foreach ($node->tabs as $tab): ?>
  <li><a href="#<?php print $tab->field_tab_headline[LANGUAGE_NONE][0]['machine']; ?>"><?php print $tab->field_tab_headline[LANGUAGE_NONE][0]['human']; ?></a>
    <ol>
...
<?php endforeach; ?>
  <?php if ($node->has_gallery) : ?>
    <li class='tab'><a href="#galleri">Galleri</a></li>
  <?php endif; ?>
</ol>
<?php endif; ?>
...
<div id="tab-container" class="tabs">
  <ul class='tabs'>
    <?php foreach ($node->tab_names as $machine => $tab): ?>
    <li class='tab'><a href="#<?php print $machine; ?>"><?php print $tab; ?></a></li>
    <?php endforeach; ?>
    <?php if ($node->has_gallery) : ?>
     <li class='tab'><a href="#galleri">Galleri</a></li>
    <?php endif; ?>
  ...
  <?php foreach ($node->tabs as $tab): ?>
  <div id="<?php print $tab->field_tab_headline[LANGUAGE_NONE][0]['machine']; ?>">
    <?php $swap = 0; ?>
    <?php foreach ($tab->reviews as $review): ?>
    ...
    <?php endforeach; ?>
    <?php if ($tab->next_link) : ?>
    <p><a href="#<?php print $tab->next_link['machine']; ?>"><?php print t('Continue to %tab', array('%tab' => $tab->next_link['human'])); ?> <i class="fa fa-arrow-right"></i></a></p>
    <?php endif; ?>
  </div>
  <?php endforeach; ?>
  <?php if ($node->has_gallery) : ?>
