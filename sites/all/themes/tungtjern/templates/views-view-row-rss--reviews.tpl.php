...
<item>
  <title><![CDATA[<?php print $node->title; ?> [<?php print $node->field_rating[LANGUAGE_NONE][0]['value']; ?>/10]]]></title>
  ...
  <?php endif; ?>
  <description><![CDATA[<?php if (strlen($node->body[LANGUAGE_NONE][0]['summary']) > 0) : ?><?php print truncate_utf8(strip_tags($node->body[LANGUAGE_NONE][0]['summary']), 600, TRUE); ?><?php else: ?><?php print truncate_utf8(str_replace(array("\r", "\n"), '', strip_tags($node->body[LANGUAGE_NONE][0]['safe_value'])), 600, TRUE, TRUE); ?><?php endif; ?>]]></description>
